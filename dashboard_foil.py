    ...
    if len(loaded_dfs) < 1:
        ...
        for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
            if orig_d is not None:
                ...
                if meta["months"]: d_f = d_f[d_f[c["Month"]].isin(meta_cr["months"])]
...
with tab_churn:
    ...
    if len(dfs_ch) < 2:
        ...
        kpi2.metric("Lost Value (from Churned Customers)", f"{format_number_plain(val_lost)} EUR", f"-{len(df_lost_cust)} Customers", delta_color="inverse")
        ...
        with col_new:
            st.markdown(f"#### 🟢 Top 50 New Customers ({y_new})")
            df_new_cust = df_new_cust.sort_values("Net_New", ascending=False).head(50)
        ...
        with col_lost:
            st.markdown(f"#### 🔴 Top 50 Lost Customers (Bought in {y_old}, 0 in {y_new})")
            df_lost_cust = df_lost_cust.sort_values("Net_Old", ascending=False).head(50)
