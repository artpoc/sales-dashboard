...
# Z palety usunięto jasny 'Swan White' (#f7f1e3), aby wykresy były wyraźne.
SPANISH_PALETTE = [
...
with tab_l4l:
    ...
    if len(year_ops) < 2: st.warning("Requires at least two year files.")
        ...
        if left_y == right_y: st.error("Years must be different.")
            ...
            # Explicit call to render Two Year Dashboard (includes Customer Impact)
...
with tab_customer:
    ...
    if not [d for d in [df_curr, df_prev, df_old2] if d is not None]: st.warning("Upload data.")
        ...
        if m_cr["country"] != "All Countries": df_for_customers_cr = df_for_customers_cr[df_for_customers_cr[hierarchy_cols["Country"]] == m_cr["country"]]
        m_cr["customer"] = c2.selectbox("🏢 Customer", ["All Customers"] + sorted(df_for_customers_cr[hierarchy_cols["Customer"]].replace("", pd.NA).dropna().unique().tolist()), key="cr_cu")
        m_cr["category"] = c3.selectbox("📦 Category", ["All Categories"] + sorted(df_for_customers_cr[hierarchy_cols["Cat"]].dropna().unique().tolist()), key="cr_ca")
        m_cr["months"] = c4.multiselect("📅 Months", MONTHS_ORDER, default=hierarchy_months or MONTHS_ORDER, key="cr_mo")

        if m_cr["customer"] == "All Customers": st.info("⚠️ Please select a specific customer from the filter above to view the dedicated analysis in this tab.")
        else:
            ...
            for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
                if orig_d is not None:
                    d_f = orig_d.copy()
                    if m_cr["country"] != "All Countries": d_f = d_f[d_f[c["Country"]] == m_cr["country"]]
                    if m_cr["customer"] != "All Customers": d_f = d_f[d_f[c["Customer"]] == m_cr["customer"]]
                    if m_cr["category"] != "All Categories": d_f = d_f[d_f[c["Cat"]] == m_cr["category"]]
                    if m_cr["months"]: d_f = d_f[d_f[c["Month"]].isin(m_cr["months"])] 
                    
                    if not d_f.empty: cr_valid_dfs.append((d_f, y, c))
            ...
            st.divider()
            default_divisor = len(m_cr["months"]) if m_cr["months"] else 12
            st.info(f"ℹ️ Averages and Sums are calculated using {default_divisor} months based on your selected month filters above.")
            avg_divisor = st.slider("Select number of months for ∑ and AVG calculation:", min_value=1, max_value=12, value=default_divisor, step=1, key="cr_slider")
            ...
            def render_monthly_table(mode="Net"):
                ...
                for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
                    if orig_d is not None:
                        ...
                        m_vals = {}
                        for m in MONTHS_ORDER: m_vals[m] = sum_decimal(d_f[d_f[c["Month"]] == m][c[col_key]])
                        
                        partial_sum = sum(m_vals[m] for m in MONTHS_ORDER[:avg_divisor])
                ...
                for y in available_years:
                    row = {"Year": str(y)}
                    for m in col_keys: row[SHORT_MONTHS.get(m, m)] = to_display_num(year_data[y][m])
                    display_rows.append(row)

                if len(available_years) >= 2:
                    y_newest, y1 = available_years[-1], available_years[-2]
                    row_yoy = {"Year": f"∆ {y_newest} vs {y1}"}
                    for m in col_keys:
                        disp_name = SHORT_MONTHS.get(m, m)
                        if m in m_cr["months"] or m in [sum_col_key, avg_col_key]: row_yoy[disp_name] = yoy_label(yoy_calc(year_data[y_newest][m], year_data[y1][m]))
                        else: row_yoy[disp_name] = "n/a"
                    display_rows.append(row_yoy)

                st.dataframe(style_monthly_table(pd.DataFrame(display_rows)), use_container_width=True, hide_index=True)
            ...
            for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
                if orig_d is not None:
                    ...
                    if m_cr["category"] != "All Categories": d_f = d_f[d_f[c["Cat"]] == m_cr["category"]]
                    if m_cr["months"]: d_f = d_f[d_f[c["Month"]].isin(m_cr["months"])]
                    if not d_f.empty: chart_vals.append({"Year": str(y), "Net": float(sum_decimal(d_f[c["Net"]]))})
            if chart_vals: st.plotly_chart(px.bar(pd.DataFrame(chart_vals).sort_values("Year"), x="Year", y="Net", text="Net", color="Year", color_discrete_map=GLOBAL_COLOR_MAP), use_container_width=True)
            ...
            def render_cr_sub_analysis(group_col_key, section_title, display_name):
                ...
                for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
                    if orig_d is not None:
                        ...
                        if m_cr["country"] != "All Countries": d_f = d_f[d_f[c["Country"]] == m_cr["country"]]
                        if m_cr["customer"] != "All Customers": d_f = d_f[d_f[c["Customer"]] == m_cr["customer"]]
                        if group_col_key != "Cat" and m_cr["category"] != "All Categories": d_f = d_f[d_f[c["Cat"]] == m_cr["category"]]
                        if m_cr["months"]: d_f = d_f[d_f[c["Month"]].isin(m_cr["months"])]
                        if not d_f.empty: group_dfs.append((d_f.groupby(c[group_col_key]).agg({c["Net"]: sum_decimal}).reset_index().rename(columns={c["Net"]: f"Net {y}"}), y, c[group_col_key]))

                if group_dfs:
                    g_col_name = group_dfs[0][2]
                    master = group_dfs[0][0]
                    for g, y, _ in group_dfs[1:]: master = pd.merge(master, g, on=g_col_name, how="outer")
                    master = master.fillna(Decimal('0'))
                    ...
                    for i, (g, y, _) in enumerate(group_dfs_chrono):
                        plot_df = master.copy()
                        plot_df[f"Net {y}"] = plot_df.get(f"Net {y}").apply(lambda v: max(0, float(clean_number(v))))
                        if plot_df[f"Net {y}"].sum() > 0:
                            plot_df['Share'] = plot_df[f"Net {y}"] / plot_df[f"Net {y}"].sum()
                    ...
                    for _, y, _ in group_dfs_chrono: display_df[f"Net {y}"] = display_df.get(f"Net {y}").apply(to_display_num)
                    if len(group_dfs_chrono) >= 2:
                        y1 = group_dfs_chrono[-2][1]
                    ...
                    cols_order = [display_name] + [f"Net {y}" for _,y,_ in group_dfs_chrono] + ([f"YoY {y_newest} vs {group_dfs_chrono[-2][1]} (%)"] if len(group_dfs_chrono)>=2 else [])
                    st.dataframe(add_index(display_df[cols_order]), use_container_width=True)

            if m_cr["category"] == "All Categories": render_cr_sub_analysis("Cat", "Category Comparison", "Category")
            ...
            for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
                if orig_d is not None:
                    ...
                    if m_cr["months"]: d_f = d_f[d_f[c["Month"]].isin(m_cr["months"])]
                    if not d_f.empty:
                        l4l_dfs.append((d_f.groupby(c["Code"]).agg({c["Desc"]: "first", c["Net"]: sum_decimal, c["Qty"]: sum_decimal}).reset_index().rename(columns={c["Net"]: f"Net {y}", c["Qty"]: f"Qty {y}"}), y, c))
                    
            if l4l_dfs:
                c_code, c_desc = l4l_dfs[0][2]["Code"], hierarchy_cols["Desc"]
...
with tab_country:
    ...
    if not [d for d in [df_curr, df_prev, df_old2] if d is not None]: st.warning("Upload data.")
        ...
        m_co["brand"] = c2.selectbox("🏷️ Brand (Licence)", ["All Brands"] + sorted(df_all_co[hierarchy_cols["Brand"]].dropna().unique().tolist()), key="co_br")
        ...
        for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
            if orig_d is not None:
                ...
                if m_co["category"] != "All Categories": d_f = d_f[d_f[c["Cat"]] == m_co["category"]]
                if m_co["brand"] != "All Brands": d_f = d_f[d_f[c["Brand"]] == m_co["brand"]]
                if m_co["months"]: d_f = d_f[d_f[c["Month"]].isin(m_co["months"])] 
                if not d_f.empty: co_valid_dfs.append((d_f, y, c))
        
        co_valid_dfs_chrono = sorted(co_valid_dfs, key=lambda x: x[1])

        if not co_valid_dfs_chrono: st.warning("No data matches selected criteria.")
        else:
            co_dfs_grouped = []
            ...
            for g, y_str in co_dfs_grouped[1:]: master_co = pd.merge(master_co, g, on="Country", how="outer")
            ...
            for y_str in [item[1] for item in co_dfs_grouped]:
                for _, r in master_co.iterrows():
                    ...
                    if val != 0: chart_data_rows.append({"Year": y_str, "Country": r["Country"], "Net": val})
            ...
            else: st.info("No net values to display.")

            st.markdown("### Country Performance (L4L)")
            co_years_str = [item[1] for item in co_dfs_grouped]
            display_cols_co = ["Country"]
            for y_str in co_years_str: display_cols_co.extend([f"Net {y_str}", f"Qty {y_str}"])
            ...
            for y_str in co_years_str:
                master_co_disp[f"Net {y_str}"] = master_co_disp.get(f"Net {y_str}").apply(to_display_num)
                master_co_disp[f"Qty {y_str}"] = master_co_disp.get(f"Qty {y_str}").apply(to_display_num)
            ...
            for i, (g, y_str) in enumerate(co_dfs_grouped):
                ...
                plot_df[f"Net {y_str}"] = plot_df.get(f"Net {y_str}").apply(lambda v: max(0, float(clean_number(v))))
                if plot_df[f"Net {y_str}"].sum() > 0:
                    plot_df['Share'] = plot_df[f"Net {y_str}"] / plot_df[f"Net {y_str}"].sum()
                    plot_df.loc[plot_df['Share'] < 0.005, "Country"] = 'Other'
                    plot_df = plot_df.groupby("Country", as_index=False)[[f"Net {y_str}"]].sum()
                with pie_cols_co[i]: st.plotly_chart(px.pie(plot_df, names="Country", values=f"Net {y_str}", title=f"Country Share {y_str}", color="Country", color_discrete_map=GLOBAL_COLOR_MAP), use_container_width=True)
            ...
            if len(co_years_str) >= 2:
                ...
                display_cols_ins = ["Country"] + [f"Net {y}" for y in co_years_str] + [f"Change {y_newest_str} vs {y1_str}", f"YoY {y_newest_str} vs {y1_str} (%)"]
                ...
                with c1:
                    ...
                    if not growth.empty:
                        disp = growth.copy()
                        for y_str in co_years_str: disp[f"Net {y_str}"] = disp.get(f"Net {y_str}").apply(to_display_num)
...
with tab_brand:
    ...
    if not [d for d in [df_curr, df_prev, df_old2] if d is not None]: st.warning("Upload data.")
        ...
        m_br["customer"] = c2.selectbox("🏢 Customer", ["All Customers"] + sorted(df_for_brands[hierarchy_cols["Customer"]].replace("", pd.NA).dropna().unique().tolist()), key="br_cu")
        m_br["category"] = c3.selectbox("📦 Category", ["All Categories"] + sorted(df_for_brands[hierarchy_cols["Cat"]].dropna().unique().tolist()), key="br_ca")
        selected_brand_specific = c4.selectbox("🏷️ Brand (Licence)", ["All Brands"] + sorted(df_for_brands[hierarchy_cols["Brand"]].dropna().unique().tolist()), key="br_brand_specific")
        m_br["months"] = st.multiselect("📅 Months", MONTHS_ORDER, default=hierarchy_months or MONTHS_ORDER, key="br_mo")

        if selected_brand_specific == "All Brands": st.info("⚠️ Please select a specific Brand from the filter above to view the dedicated analysis.")
        else:
            ...
            for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
                if orig_d is not None:
                    d_f = orig_d.copy()
                    if m_br["country"] != "All Countries": d_f = d_f[d_f[c["Country"]] == m_br["country"]]
                    if m_br["customer"] != "All Customers": d_f = d_f[d_f[c["Customer"]] == m_br["customer"]]
                    if m_br["category"] != "All Categories": d_f = d_f[d_f[c["Cat"]] == m_br["category"]]
                    d_f = d_f[d_f[c["Brand"]] == selected_brand_specific]
                    if m_br["months"]: d_f = d_f[d_f[c["Month"]].isin(m_br["months"])] 
                    if not d_f.empty: br_valid_dfs.append((d_f, y, c))
            ...
            if len(br_valid_dfs_chrono) >= 2:
                ...
                s_new_net, s_old_net = sum_decimal(d_new[c_new["Net"]]), sum_decimal(d_old[c_old["Net"]])
                s_new_qty, s_old_qty = sum_decimal(d_new[c_new["Qty"]]), sum_decimal(d_old[c_old["Qty"]])
                ...
                kc3.metric(f"Qty {y_old} (PCS)", format_number_plain(s_old_qty))
                kc4.metric(f"Qty {y_new} (PCS)", format_number_plain(s_new_qty), yoy_label(yoy_calc(s_new_qty, s_old_qty)))
            else: st.info("Not enough data to calculate comparison KPIs (need at least 2 years).")
                
            st.divider()
            default_divisor_br = len(m_br["months"]) if m_br["months"] else 12
            ...
            def render_brand_monthly_table(mode="Net"):
                st.markdown(f"### {mode} Value Monthly Comparison")
                col_key = "Net" if mode == "Net" else "Qty"
                sum_col_key, avg_col_key = f"∑ (for {avg_divisor_br} months)", "Avg Month"
                ...
                for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
                    if orig_d is not None:
                        ...
                        partial_sum = sum(m_vals[m] for m in MONTHS_ORDER[:avg_divisor_br])
                        m_vals[sum_col_key], m_vals[avg_col_key] = partial_sum, partial_sum / Decimal(str(avg_divisor_br))
                        year_data[y] = m_vals
                ...
                if len(available_years) >= 2:
                    ...
                    for m in col_keys:
                        disp_name = SHORT_MONTHS.get(m, m)
                        if m in m_br["months"] or m in [sum_col_key, avg_col_key]: row_yoy[disp_name] = yoy_label(yoy_calc(year_data[y_newest][m], year_data[y1][m]))
                        else: row_yoy[disp_name] = "n/a"
                    display_rows.append(row_yoy)
            ...
            st.divider()
            st.markdown("### Net Value Comparison")
            chart_vals_br = [{"Year": str(y), "Net": float(sum_decimal(d_f[c["Net"]]))} for d_f, y, c in br_valid_dfs_chrono]
            if chart_vals_br: st.plotly_chart(px.bar(pd.DataFrame(chart_vals_br).sort_values("Year"), x="Year", y="Net", text="Net", color="Year", color_discrete_map=GLOBAL_COLOR_MAP), use_container_width=True, key="br_net_bar")
...
with tab_churn:
    ...
    if len([d for d in [df_curr, df_prev, df_old2] if d is not None]) < 2: st.warning("Please upload at least 2 years of data to analyze Churn & Acquisition.")
    else:
        ...
        m_ch["country"] = ch1.selectbox("🌎 Country", ["All Countries"] + sorted(df_all_ch[hierarchy_cols["Country"]].replace("", pd.NA).dropna().unique().tolist()), key="ch_co")
        m_ch["category"] = ch2.selectbox("📦 Category", ["All Categories"] + sorted(df_all_ch[hierarchy_cols["Cat"]].dropna().unique().tolist()), key="ch_ca")
        m_ch["brand"] = ch3.selectbox("🏷️ Brand", ["All Brands"] + sorted(df_all_ch[hierarchy_cols["Brand"]].dropna().unique().tolist()), key="ch_br")
        m_ch["months"] = ch4.multiselect("📅 Months", MONTHS_ORDER, default=hierarchy_months or MONTHS_ORDER, key="ch_mo")

        ch_valid_dfs = []
        for orig_d, y, c in zip([df_old2, df_prev, df_curr], [year_old2, year_prev, year_curr], [cols_old2, cols_prev, cols_curr]):
            if orig_d is not None:
                d_f = orig_d.copy()
                if m_ch["country"] != "All Countries": d_f = d_f[d_f[c["Country"]] == m_ch["country"]]
                if m_ch["category"] != "All Categories": d_f = d_f[d_f[c["Cat"]] == m_ch["category"]]
                if m_ch["brand"] != "All Brands": d_f = d_f[d_f[c["Brand"]] == m_ch["brand"]]
                if m_ch["months"]: d_f = d_f[d_f[c["Month"]].isin(m_ch["months"])] 
                if not d_f.empty: ch_valid_dfs.append((d_f, y, c))
        ...
        if len(ch_valid_dfs_chrono) >= 2:
            ...
            g_n = d_new.groupby(c_new["Customer"]).agg({c_new["Net"]: sum_decimal}).reset_index().rename(columns={c_new["Customer"]: "Customer", c_new["Net"]: "Net_New"})
            g_o = d_old.groupby(c_old["Customer"]).agg({c_old["Net"]: sum_decimal}).reset_index().rename(columns={c_old["Customer"]: "Customer", c_old["Net"]: "Net_Old"})
            ...
            net_impact = v_acq - v_los
            
            st.divider()
            st.markdown(f"### Acquisition & Churn KPIs ({y_old} ➔ {y_new})")
            kpi1, kpi2, kpi3 = st.columns(3)
            kpi1.metric("Acquired Value (from New Customers)", f"{format_number_plain(v_acq)} EUR", f"{len(df_new_c)} Customers")
            kpi2.metric("Lost Value (from Churned Customers)", f"{format_number_plain(v_los)} EUR", f"-{len(df_lost_c)} Customers", delta_color="normal")
            ...
            with col_new:
                ...
                df_new_c = df_new_c.sort_values("Net_New", ascending=False).head(10)
                disp_new = df_new_c[["Customer", "Net_New"]].copy()
                disp_new["Net_New"] = disp_new["Net_New"].apply(to_display_num)
                disp_new = disp_new.rename(columns={"Net_New": f"Net {y_new} (EUR)"})
                if disp_new.empty: st.info("No new customers found for this criteria.")
                else: st.dataframe(add_index(disp_new), use_container_width=True)
                    
            with col_lost:
                st.markdown(f"#### 🔴 Top 10 Lost Customers (Bought in {y_old}, 0 in {y_new})")
                df_lost_c = df_lost_c.sort_values("Net_Old", ascending=False).head(10)
                disp_lost = df_lost_c[["Customer", "Net_Old"]].copy()
                ...
                if disp_lost.empty: st.success("No churned customers found for this criteria!")
                else: st.dataframe(add_index(disp_lost), use_container_width=True)
        else:
