...
def clean_number(x):
    ...
    if x is None or pd.isna(x): return Decimal('0')
    s = str(x).replace("\xa0","").replace("\u202f","").replace(" ","").replace(",", ".")
    if s in ["", "-", "nan", "None"]: return Decimal('0')
    try: return Decimal(s)
    except InvalidOperation: return Decimal('0')

def sum_decimal(series):
    total = Decimal('0')
    for v in series: total += clean_number(v)
    return total

def to_int(d):
    try: return int(clean_number(d).quantize(Decimal("1"), rounding=ROUND_HALF_UP))
    except: return 0
...
SHORT_MONTHS = {"January": "JAN", "February": "FEB", "March": "MAR", "April": "APR", "May": "MAY", "June": "JUN", "July": "JUL", "August": "AUG", "September": "SEP", "October": "OCT", "November": "NOV", "December": "DEC", "Total": "∑", "Avg Month": "AVG / Month"}

SPANISH_PALETTE = ["#40407a", "#706fd3", "#34ace0", "#33d9b2", "#2c2c54", "#474787", "#aaa69d", "#227093", "#218c74", "#ff5252", "#ff793f", "#d1ccc0", "#ffb142", "#ffda79", "#b33939", "#cd6133", "#84817a", "#cc8e35", "#ccae62"]
...
def normalize_month(x) -> str:
    ...
    exact_map = {"1": "January", "01": "January", "january": "January", "styczeń": "January", "styczen": "January", "2": "February", "02": "February", "february": "February", "luty": "February", "3": "March", "03": "March", "march": "March", "marzec": "March", "4": "April", "04": "April", "april": "April", "kwiecień": "April", "kwiecien": "April", "5": "May", "05": "May", "may": "May", "maj": "May", "6": "June", "06": "June", "june": "June", "czerwiec": "June", "7": "July", "07": "July", "july": "July", "lipiec": "July", "8": "August", "08": "August", "august": "August", "sierpień": "August", "sierpien": "August", "9": "September", "09": "September", "september": "September", "wrzesień": "September", "wrzesien": "September", "10": "October", "october": "October", "październik": "October", "pazdziernik": "October", "11": "November", "november": "November", "listopad": "November", "12": "December", "december": "December", "grudzień": "December", "grudzien": "December"}
    if x_str in exact_map: return exact_map[x_str]
    ...
    try: return pd.to_datetime(x_str).strftime("%B")
    except: return str(x).strip().capitalize()
...
def format_number_plain(d, decimals: int = DISPLAY_DECIMALS) -> str:
    d = clean_number(d)
    try:
        if decimals == 0: return f"{int(d.quantize(Decimal('1'), rounding=ROUND_HALF_UP))}"
        return f"{d.quantize(Decimal('0.1'), rounding=ROUND_HALF_UP):.1f}"
    except: return "0"

def to_display_num(d):
    d = clean_number(d)
    try:
        if DISPLAY_DECIMALS == 0: return int(d.quantize(Decimal('1'), rounding=ROUND_HALF_UP))
...
ALLOWED_CATEGORIES = ["Napkins", "Hats", "Banner", "Straws", "Bags", "Plates", "Paper Cups", "Tablecover", "Reusable", "Foil", "Wooden", "Candles", "Latex", "Invitations", "Masks", "Pinata", "Plastic Cups", "Horns", "Other"]
...
def extract_year_from_header(col_name: str) -> str:
    ...
    return digits if digits else str(col_name)
...
def load_single_year_file(file, label: str):
    ...
    month_col = next((c for c in df.columns if any(x in str(c).lower() for x in ["month", "mies", "m-c"])), df.columns[0])
    customer_col = next((c for c in df.columns if any(x in str(c).lower() for x in ["customer", "klient", "odbiorca", "nabywca"])), df.columns[1] if len(df.columns) > 1 else df.columns[0])
    country_col = next((c for c in df.columns if any(x in str(c).lower() for x in ["country", "kraj"])), df.columns[2] if len(df.columns) > 2 else df.columns[0])
    code_col = next((c for c in df.columns if ("art" in str(c).lower() and "nr" in str(c).lower()) or "code" in str(c).lower()), df.columns[4] if len(df.columns) > 4 else df.columns[0])
    desc_col = next((c for c in df.columns if any(x in str(c).lower() for x in ["description", "opis"])), df.columns[5] if len(df.columns) > 5 else df.columns[0])
    brand_col = next((c for c in df.columns if any(x in str(c).lower() for x in ["brand", "marka", "licencja"])), df.columns[6] if len(df.columns) > 6 else df.columns[0])
    cat_col = next((c for c in df.columns if any(x in str(c).lower() for x in ["category", "kategoria"])), df.columns[7] if len(df.columns) > 7 else df.columns[0])

    net_col, qty_col = None, None
    for c in df.columns:
        cl = str(c).lower()
        if net_col is None and any(x in cl for x in ["net", "value", "sales", "warto"]): net_col = c
        elif qty_col is None and any(x in cl for x in ["qty", "quantity", "pcs", "ilo"]): qty_col = c
    ...
    df = df[df["Desc_Clean"].notna() & (df["Desc_Clean"].str.lower() != "none")]

    year_name = extract_year_from_header(net_col)
    cols = {"YearName": year_name, "Month": "Month_Clean", "Customer": "Customer_Clean", "Country": "Country_Clean", "Code": "Code_Clean", "Desc": "Desc_Clean", "Brand": "Brand_Clean", "Cat": "Category_Clean", "Net": net_col, "Qty": qty_col}
    return df, cols, year_name

def create_single_filters(df: pd.DataFrame, cols: dict, unique_prefix: str):
    if df is None or cols is None: return None, None
    df_all = df.copy()
    
    countries = ["All Countries"] + sorted(df_all[cols["Country"]].replace("", pd.NA).dropna().unique().tolist())
    selected_country = st.selectbox("Country", countries, key=f"{unique_prefix}_country")

    df_for_customers = df_all.copy()
    if selected_country != "All Countries": df_for_customers = df_for_customers[df_for_customers[cols["Country"]] == selected_country]

    customers = ["All Customers"] + sorted(df_for_customers[cols["Customer"]].replace("", pd.NA).dropna().unique().tolist())
    selected_customer = st.selectbox("Customer", customers, key=f"{unique_prefix}_customer")

    categories = ["All Categories"] + sorted(df_for_customers[cols["Cat"]].dropna().unique().tolist())
    selected_category = st.selectbox("Category", categories, key=f"{unique_prefix}_category")

    df_filtered = df_all.copy()
    if selected_country != "All Countries": df_filtered = df_filtered[df_filtered[cols["Country"]] == selected_country]
    if selected_customer != "All Customers": df_filtered = df_filtered[df_filtered[cols["Customer"]] == selected_customer]
    if selected_category != "All Categories": df_filtered = df_filtered[df_filtered[cols["Cat"]] == selected_category]

    return df_filtered, {"country": selected_country, "customer": selected_customer, "category": selected_category, "df_all": df_all}

def apply_shared_filters(dfs, cols, unique_prefix: str, default_months=None, show_months=True):
    if not dfs: return [], None
    df_all = pd.concat(dfs, ignore_index=True)
    c1, c2, c3, c4 = st.columns(4) if show_months else (*st.columns(3), None)

    countries = ["All Countries"] + sorted(df_all[cols["Country"]].replace("", pd.NA).dropna().unique().tolist())
    selected_country = c1.selectbox("🌎 Country", countries, key=f"{unique_prefix}_country")

    df_for_customers = df_all.copy()
    if selected_country != "All Countries": df_for_customers = df_for_customers[df_for_customers[cols["Country"]] == selected_country]

    customers = ["All Customers"] + sorted(df_for_customers[cols["Customer"]].replace("", pd.NA).dropna().unique().tolist())
    selected_customer = c2.selectbox("🏢 Customer", customers, key=f"{unique_prefix}_customer")

    categories = ["All Categories"] + sorted(df_for_customers[cols["Cat"]].dropna().unique().tolist())
    selected_category = c3.selectbox("📦 Category", categories, key=f"{unique_prefix}_category")

    if show_months:
        options_m = MONTHS_ORDER
        default_m = [m for m in (default_months or MONTHS_ORDER) if m in options_m]
        selected_months = c4.multiselect("📅 Months", options=options_m, default=default_m, key=f"{unique_prefix}_months")
    else:
        selected_months = default_months or MONTHS_ORDER

    filtered_dfs = []
    for df in dfs:
        d = df.copy()
        if selected_country != "All Countries": d = d[d[cols["Country"]] == selected_country]
        if selected_customer != "All Customers": d = d[d[cols["Customer"]] == selected_customer]
        if selected_category != "All Categories": d = d[d[cols["Cat"]] == selected_category]
        if selected_months and show_months: d = d[d[cols["Month"]].isin(selected_months)]
        filtered_dfs.append(d)

    return filtered_dfs, {"country": selected_country, "customer": selected_customer, "category": selected_category, "months": selected_months, "df_all": df_all}

def render_two_year_dashboard(df_new, df_old, cols_new, cols_old, context_name, unique_prefix, category_filter="All Categories", color_map=None):
    color_map = color_map or {}
    if df_new is None or df_old is None:
        st.warning("No data for this view.")
        return

    net_new, qty_new, code_col, desc_col, brand_col, cust_col, cat_col = cols_new["Net"], cols_new["Qty"], cols_new["Code"], cols_new["Desc"], cols_new["Brand"], cols_new["Customer"], cols_new["Cat"]
    net_old, qty_old = cols_old["Net"], cols_old["Qty"]
    year_new, year_old = cols_new["YearName"], cols_old["YearName"]

    st.markdown(f"### KPI ({year_old} vs {year_new})")
    s_new, s_old = sum_decimal(df_new[net_new]), sum_decimal(df_old[net_old])
    q_new, q_old = sum_decimal(df_new[qty_new]), sum_decimal(df_old[qty_old])

    c1, c2, c3, c4 = st.columns(4)
    c1.metric(f"Net {year_old} (EUR)", format_number_plain(s_old))
    c2.metric(f"Net {year_new} (EUR)", format_number_plain(s_new), yoy_label(yoy_calc(s_new, s_old)))
    c3.metric(f"Qty {year_old} (PCS)", format_number_plain(q_old))
    c4.metric(f"Qty {year_new} (PCS)", format_number_plain(q_new), yoy_label(yoy_calc(q_new, q_old)))

    st.divider()
    if category_filter == "All Categories":
        st.markdown("### Category Performance")
        cat_new_df = df_new.groupby(cat_col).agg({net_new: sum_decimal}).reset_index().rename(columns={net_new: f"Net {year_new}"})
        cat_old_df = df_old.groupby(cat_col).agg({net_old: sum_decimal}).reset_index().rename(columns={net_old: f"Net {year_old}"})
        cat = pd.merge(cat_new_df, cat_old_df, on=cat_col, how="outer").fillna(Decimal('0'))
        cat[f"YoY {year_new} vs {year_old}"] = cat.apply(lambda x: yoy_calc(x.get(f"Net {year_new}", Decimal('0')), x.get(f"Net {year_old}", Decimal('0'))), axis=1) if not cat.empty else []
        cat = sort_by_col_desc(cat, f"Net {year_new}")

        plot_cat = cat.copy()
        plot_cat[f"Net {year_new}"] = plot_cat.get(f"Net {year_new}", pd.Series(dtype=float)).apply(lambda v: max(0, float(clean_number(v))))
        plot_cat[f"Net {year_old}"] = plot_cat.get(f"Net {year_old}", pd.Series(dtype=float)).apply(lambda v: max(0, float(clean_number(v))))

        pc1, pc2 = st.columns(2)
        with pc1:
            st.markdown(f"#### Category Pie {year_old}")
            st.plotly_chart(px.pie(plot_cat, names=cat_col, values=f"Net {year_old}", color=cat_col, color_discrete_map=color_map), use_container_width=True)
        with pc2:
            st.markdown(f"#### Category Pie {year_new}")
            st.plotly_chart(px.pie(plot_cat, names=cat_col, values=f"Net {year_new}", color=cat_col, color_discrete_map=color_map), use_container_width=True)
        ...
        st.dataframe(add_index(cat_display[[cat_col, f"Net {year_old}", f"Net {year_new}", "YoY (%)"]]))
        st.divider()

    st.markdown("### Brand Performance")
    brand_new_df = df_new.groupby(brand_col).agg({net_new: sum_decimal}).reset_index().rename(columns={net_new: f"Net {year_new}"})
    brand_old_df = df_old.groupby(brand_col).agg({net_old: sum_decimal}).reset_index().rename(columns={net_old: f"Net {year_old}"})
    brand = pd.merge(brand_new_df, brand_old_df, on=brand_col, how="outer").fillna(Decimal('0'))
    brand[f"YoY {year_new} vs {year_old}"] = brand.apply(lambda x: yoy_calc(x.get(f"Net {year_new}", Decimal('0')), x.get(f"Net {year_old}", Decimal('0'))), axis=1) if not brand.empty else []
    brand = sort_by_col_desc(brand, f"Net {year_new}")

    bplot = brand.copy()
    bplot[f"Net {year_old}"] = bplot.get(f"Net {year_old}", pd.Series(dtype=float)).apply(lambda v: max(0, float(clean_number(v))))
    bplot[f"Net {year_new}"] = bplot.get(f"Net {year_new}", pd.Series(dtype=float)).apply(lambda v: max(0, float(clean_number(v))))

    bc1, bc2 = st.columns(2)
    with bc1:
        st.markdown(f"#### Brand Pie {year_old}")
        st.plotly_chart(px.pie(bplot, names=brand_col, values=f"Net {year_old}", color=brand_col, color_discrete_map=color_map), use_container_width=True)
    with bc2:
        st.markdown(f"#### Brand Pie {year_new}")
        st.plotly_chart(px.pie(bplot, names=brand_col, values=f"Net {year_new}", color=brand_col, color_discrete_map=color_map), use_container_width=True)
    ...
    if df_new.empty and df_old.empty: st.info("No data for selected filters.")
    else:
        t1, t2 = st.columns(2)
        with t1:
            st.write(f"#### Top 10 by Net {year_old}")
            d_old = df_old.groupby(code_col).agg({desc_col: "first", net_old: sum_decimal, qty_old: sum_decimal}).reset_index()
            d_old = sort_by_col_desc(d_old[d_old[net_old].apply(lambda x: clean_number(x) > 0)], net_old)
            if d_old.empty: st.info("No sales in older year.")
            else:
                top_old = d_old.head(10)
                tot_old = sum_decimal(d_old[net_old])
                top_old["Share (%)"] = top_old[net_old].apply(lambda x: percent_label(clean_number(x) / tot_old * Decimal('100'))) if tot_old else 0.0
        ...
        with t2:
            ...
            d_new = df_new.groupby(code_col).agg({desc_col: "first", net_new: sum_decimal, qty_new: sum_decimal}).reset_index()
            d_new = sort_by_col_desc(d_new[d_new[net_new].apply(lambda x: clean_number(x) > 0)], net_new)
            if d_new.empty: st.info("No sales in newer year.")
            else:
                top_new = d_new.head(10)
                tot_new = sum_decimal(d_new[net_new])
                top_new["Share (%)"] = top_new[net_new].apply(lambda x: percent_label(clean_number(x) / tot_new * Decimal('100'))) if tot_new else 0.0
    ...
    for tab, year, df_src, net_col in zip(st.tabs([year_old, year_new]), [year_old, year_new], [df_old, df_new], [net_old, net_new]):
        with tab:
            p = df_src.groupby(code_col).agg({desc_col: "first", cat_col: "first", net_col: sum_decimal}).reset_index()
            p = sort_by_col_desc(p[p[net_col].apply(lambda x: clean_number(x) > 0)], net_col)
            if p.empty: st.info("No sales in this period.")
            else:
                p["cum_value"] = p[net_col].cumsum()
                tot_val = sum_decimal(p[net_col])
                p["cum_share"] = p["cum_value"].apply(lambda x: clean_number(x) / tot_val) if tot_val else 0
                top80 = p[p["cum_share"] <= Decimal('0.8')]
                st.write(f"Top SKU for 80%: {top80[code_col].nunique()} / {p[code_col].nunique()}")
    ...
    for tab, year, df_src, net_col in zip(st.tabs([year_old, year_new]), [year_old, year_new], [df_old, df_new], [net_old, net_new]):
        with tab:
            a = df_src.groupby(code_col).agg({desc_col: "first", net_col: sum_decimal}).reset_index()
            a = sort_by_col_desc(a[a[net_col].apply(lambda x: clean_number(x) > 0)], net_col).reset_index(drop=True)
            if a.empty: st.info("No sales in this period.")
            else:
                tot_val = sum_decimal(a[net_col])
                a["cum"] = a[net_col].cumsum().apply(lambda x: clean_number(x) / tot_val) if tot_val else 0
                a["segment"] = "C"
    ...
    yoy = pd.merge(
        df_new.groupby(code_col).agg({desc_col: "first", net_new: sum_decimal, qty_new: sum_decimal}).reset_index(),
        df_old.groupby(code_col).agg({desc_col: "first", net_old: sum_decimal, qty_old: sum_decimal}).reset_index(),
        on=code_col, how="outer", suffixes=("", "_old")
    )
    yoy[desc_col] = yoy[desc_col].fillna(yoy[f"{desc_col}_old"])
    yoy = yoy.drop(columns=[f"{desc_col}_old"]).fillna(Decimal('0'))
    yoy["YoY"] = yoy.apply(lambda x: yoy_calc(x.get(net_new, 0), x.get(net_old, 0)), axis=1) if not yoy.empty else []
    yoy = sort_by_col_desc(yoy, net_new)

    yoy_disp = yoy.copy()
    for col in [net_old, net_new, qty_old, qty_new]: yoy_disp[col] = yoy_disp.get(col, pd.Series(dtype=int)).apply(to_display_num)
    ...
    st.markdown("### Auto Insights (Category Focus)" if category_filter == "All Categories" else f"### Auto Insights (SKU Focus - {category_filter})")
    cat_ins = pd.merge(
        df_new.groupby(cat_col if category_filter == "All Categories" else code_col).agg({net_new: sum_decimal} if category_filter == "All Categories" else {desc_col: "first", net_new: sum_decimal}).reset_index(),
        df_old.groupby(cat_col if category_filter == "All Categories" else code_col).agg({net_old: sum_decimal} if category_filter == "All Categories" else {desc_col: "first", net_old: sum_decimal}).reset_index(),
        on=(cat_col if category_filter == "All Categories" else code_col), how="outer", suffixes=("", "_old")
    ).fillna(Decimal('0'))
    
    disp_prefix = [cat_col] if category_filter == "All Categories" else ["Code", "Description"]
    if category_filter != "All Categories":
        cat_ins[desc_col] = cat_ins[desc_col].replace(Decimal('0'), pd.NA).fillna(cat_ins[f"{desc_col}_old"]).fillna("")
    ...
    cat_ins["Change_Raw"] = cat_ins.apply(lambda x: clean_number(x.get(f"Net {year_new}", Decimal('0'))) - clean_number(x.get(f"Net {year_old}", Decimal('0'))), axis=1)
    cat_ins["YoY"] = cat_ins.apply(lambda x: yoy_calc(x.get(f"Net {year_new}", Decimal('0')), x.get(f"Net {year_old}", Decimal('0'))), axis=1) if not cat_ins.empty else []
    cat_ins = sort_by_col_desc(cat_ins, "Change_Raw")

    ic1, ic2 = st.columns(2)
    with ic1:
        st.write("#### Top 5 " + ("Categories" if category_filter == "All Categories" else "SKUs"))
        disp_old_cat = cat_ins.sort_values(f"Net {year_old}", ascending=False).head(5).copy()
        disp_old_cat[f"Net {year_old}"] = disp_old_cat.get(f"Net {year_old}").apply(to_display_num)
        st.dataframe(add_index(disp_old_cat[disp_prefix + [f"Net {year_old}"]]))
    with ic2:
        st.write("#### ")
        disp_new_cat = cat_ins.sort_values(f"Net {year_new}", ascending=False).head(5).copy()
        disp_new_cat[f"Net {year_new}"] = disp_new_cat.get(f"Net {year_new}").apply(to_display_num)
        disp_new_cat["YoY (%)"] = disp_new_cat.get("YoY").apply(yoy_label)
    ...
    if growth.empty: st.info("No growth found.")
    else:
        g_disp = growth.copy()
        g_disp[f"Net {year_old}"] = g_disp.get(f"Net {year_old}").apply(to_display_num)
        g_disp[f"Net {year_new}"] = g_disp.get(f"Net {year_new}").apply(to_display_num)
        g_disp[f"Change"] = g_disp["Change_Raw"].apply(to_display_num)
        g_disp["YoY (%)"] = g_disp.get("YoY").apply(yoy_label)
        st.dataframe(add_index(g_disp[disp_prefix + [f"Net {year_old}", f"Net {year_new}", "Change", "YoY (%)"]]))

    decline = cat_ins[cat_ins["Change_Raw"] < 0].sort_values("Change_Raw", ascending=True).head(5)
    st.write("#### Top 5 Decline (L4L)")
    if decline.empty: st.success("No decline found.")
    else:
        d_disp = decline.copy()
        d_disp[f"Net {year_old}"] = d_disp.get(f"Net {year_old}").apply(to_display_num)
        d_disp[f"Net {year_new}"] = d_disp.get(f"Net {year_new}").apply(to_display_num)
        d_disp[f"Change"] = d_disp["Change_Raw"].apply(to_display_num)
        d_disp["YoY (%)"] = d_disp.get("YoY").apply(yoy_label)
        st.dataframe(add_index(d_disp[disp_prefix + [f"Net {year_old}", f"Net {year_new}", "Change", "YoY (%)"]]))
    st.divider()

    st.markdown("### Customer Impact (Growth vs Decline)")
    sel_cat_imp = st.selectbox("Category for Impact Analysis", ["All Categories"] + sorted(list(set(df_new[cat_col].dropna().unique().tolist() + df_old[cat_col].dropna().unique().tolist()))), key=f"{unique_prefix}_imp_cat")
    sel_brand_imp = st.selectbox("Brand (License) for Impact Analysis", ["All Brands"] + sorted(list(set(df_new[brand_col].dropna().unique().tolist() + df_old[brand_col].dropna().unique().tolist()))), key=f"{unique_prefix}_imp_brand")

    df_imp_new, df_imp_old = df_new.copy(), df_old.copy()
    if sel_cat_imp != "All Categories":
        df_imp_new, df_imp_old = df_imp_new[df_imp_new[cat_col] == sel_cat_imp], df_imp_old[df_imp_old[cat_col] == sel_cat_imp]
    if sel_brand_imp != "All Brands":
        df_imp_new, df_imp_old = df_imp_new[df_imp_new[brand_col] == sel_brand_imp], df_imp_old[df_imp_old[brand_col] == sel_brand_imp]

    impact = pd.merge(
        df_imp_new.groupby(cust_col).agg({net_new: sum_decimal}).reset_index().rename(columns={net_new: f"Net {year_new}"}),
        df_imp_old.groupby(cust_col).agg({net_old: sum_decimal}).reset_index().rename(columns={net_old: f"Net {year_old}"}),
        on=cust_col, how="outer"
    ).fillna(Decimal('0'))
    
    impact["Change Value Raw"] = impact.get(f"Net {year_new}", Decimal('0')) - impact.get(f"Net {year_old}", Decimal('0'))
    impact["Special Case"] = (impact.get(f"Net {year_old}", Decimal('0')) < 0) & (impact.get(f"Net {year_new}", Decimal('0')) == 0)
    impact["YoY"] = impact.apply(lambda x: yoy_calc(x.get(f"Net {year_new}", 0), x.get(f"Net {year_old}", 0)), axis=1) if not impact.empty else []

    growth_imp = pd.concat([impact[(impact["Change Value Raw"] > 0) & (~impact["Special Case"])].sort_values("Change Value Raw", ascending=False).head(10), impact[impact["Special Case"]].head(10)])
    st.write("#### Top Growth Drivers")
    if growth_imp.empty: st.info("No growth generated by customers.")
    else:
        g_disp = growth_imp.copy()
        g_disp[f"Net {year_old}"], g_disp[f"Net {year_new}"], g_disp["Change Value"] = g_disp[f"Net {year_old}"].apply(to_display_num), g_disp[f"Net {year_new}"].apply(to_display_num), g_disp["Change Value Raw"].apply(to_display_num)
    ...
    decline_imp = pd.concat([impact[impact["Change Value Raw"] < 0].sort_values("Change Value Raw").head(10), impact[impact["Special Case"]].head(10)])
    st.write("#### Top Decline Drivers")
    if decline_imp.empty: st.success("No decline across customers.")
    else:
        d_disp = decline_imp.copy()
        d_disp[f"Net {year_old}"], d_disp[f"Net {year_new}"], d_disp["Change Value"] = d_disp[f"Net {year_old}"].apply(to_display_num), d_disp[f"Net {year_new}"].apply(to_display_num), d_disp["Change Value Raw"].apply(to_display_num)
...
def render_single_year_dashboard(df, cols, year_name, unique_prefix, category_filter, color_map=None):
    color_map = color_map or {}
    if df is None or cols is None: return st.warning("No data for this view.")

    net_col, qty_col, code_col, desc_col, brand_col, cat_col = cols["Net"], cols["Qty"], cols["Code"], cols["Desc"], cols["Brand"], cols["Cat"]

    st.markdown(f"### KPI ({year_name})")
    c1, c2 = st.columns(2)
    c1.metric(f"Net {year_name} (EUR)", format_number_plain(sum_decimal(df[net_col])))
    c2.metric(f"Qty {year_name} (PCS)", format_number_plain(sum_decimal(df[qty_col])))
    st.divider()

    if category_filter == "All Categories":
        st.markdown("### Category Performance")
        cat = sort_by_col_desc(df.groupby(cat_col).agg({net_col: sum_decimal}).reset_index().rename(columns={net_col: f"Net {year_name}"}), f"Net {year_name}")
        plot_cat = cat.copy()
        plot_cat[f"Net {year_name}"] = plot_cat.get(f"Net {year_name}").apply(lambda v: float(clean_number(v)))
        st.plotly_chart(px.pie(plot_cat, names=cat_col, values=f"Net {year_name}", color=cat_col, color_discrete_map=color_map), use_container_width=True)
        cat_disp = cat.copy()
        cat_disp[f"Net {year_name}"] = cat_disp.get(f"Net {year_name}").apply(to_display_num)
        st.dataframe(add_index(cat_disp[[cat_col, f"Net {year_name}"]]))
        st.divider()

    st.markdown("### Brand Performance")
    brand = sort_by_col_desc(df.groupby(brand_col).agg({net_col: sum_decimal}).reset_index().rename(columns={net_col: f"Net {year_name}"}), f"Net {year_name}")
    bplot = brand.copy()
    bplot[f"Net {year_name}"] = bplot.get(f"Net {year_name}").apply(lambda v: float(clean_number(v)))
    st.plotly_chart(px.pie(bplot, names=brand_col, values=f"Net {year_name}", color=brand_col, color_discrete_map=color_map), use_container_width=True)
    brand_disp = brand.copy()
    brand_disp[f"Net {year_name}"] = brand_disp.get(f"Net {year_name}").apply(to_display_num)
    st.dataframe(add_index(brand_disp[[brand_col, f"Net {year_name}"]]))
    st.divider()

    st.markdown("### Top Products")
    d = sort_by_col_desc(df.groupby(code_col).agg({desc_col: "first", net_col: sum_decimal, qty_col: sum_decimal}).reset_index(), net_col)
    if d.empty: st.info("No sales.")
    else:
        top = d.head(10)
        tot_val = sum_decimal(d[net_col])
        top["Share (%)"] = top[net_col].apply(lambda x: percent_label(clean_number(x) / tot_val * Decimal('100'))) if tot_val else 0.0
        disp = top.copy()
        disp[net_col], disp[qty_col] = disp[net_col].apply(to_display_num), disp[qty_col].apply(to_display_num)
...
if df_curr is not None and not df_curr.empty: hierarchy_df, hierarchy_cols = df_curr, cols_curr
elif df_prev is not None and not df_prev.empty: hierarchy_df, hierarchy_cols = df_prev, cols_prev
elif df_old2 is not None and not df_old2.empty: hierarchy_df, hierarchy_cols = df_old2, cols_old2
...
with tab_customer:
    ...
    if not [d for d in [df_curr, df_prev, df_old2] if d is not None]: st.warning("Upload data.")
        ...
        c1, c2, c3, c4 = st.columns(4)
        m_cr = {"prefix":"cr"}
        m_cr["country"] = c1.selectbox("🌎 Country", ["All Countries"] + sorted(df_all_cr[hierarchy_cols["Country"]].replace("", pd.NA).dropna().unique().tolist()), key="cr_country")
        m_cr["customer"] = c2.selectbox("🏢 Customer", ["All Customers"] + sorted(df_all_cr[df_all_cr[hierarchy_cols["Country"]]==m_cr["country"] if m_cr["country"]!="All Countries" else df_all_cr][hierarchy_cols["Customer"]].replace("", pd.NA).dropna().unique().tolist()), key="cr_customer")
        m_cr["category"] = c3.selectbox("📦 Category", ["All Categories"] + sorted(df_all_cr[hierarchy_cols["Cat"]].dropna().unique().tolist()), key="cr_category")
        m_cr["months"] = c4.multiselect("📅 Months", MONTHS_ORDER, default=hierarchy_months or MONTHS_ORDER, key="cr_months")
        ...
        if m_cr["customer"] == "All Customers": st.info("⚠️ Select a specific customer.")
            ...
            st.divider()
            render_monthly_comparison(v_dfs, m_cr, "Net")
            st.divider()
            render_monthly_comparison(v_dfs, m_cr, "Qty")
...
with tab_country:
    ...
    if not [d for d in [df_curr, df_prev, df_old2] if d is not None]: st.warning("Upload data.")
        ...
        m_co = {"prefix": "co", "customer": "All Customers"} # Virtual for get_filtered
        m_co["category"] = c1.selectbox("📦 Category", ["All Categories"] + sorted(df_all_co[hierarchy_cols["Cat"]].dropna().unique().tolist()), key="co_category")
        m_co["brand"] = c2.selectbox("🏷️ Brand", ["All Brands"] + sorted(df_all_co[hierarchy_cols["Brand"]].dropna().unique().tolist()), key="co_brand")
        m_co["months"] = c3.multiselect("📅 Months", MONTHS_ORDER, default=hierarchy_months or MONTHS_ORDER, key="co_months")
        m_co["country"] = "All Countries" # Virtual, for filtering
...
with tab_brand:
    ...
    if not [d for d in [df_curr, df_prev, df_old2] if d is not None]: st.warning("Upload data.")
        ...
        c1, c2, c3, c4 = st.columns(4)
        m_br = {"prefix": "br"}
        m_br["country"] = c1.selectbox("🌎 Country", ["All Countries"] + sorted(df_all_br[hierarchy_cols["Country"]].replace("", pd.NA).dropna().unique().tolist()), key="br_country")
        df_for_brands = df_all_br.copy()
        if m_br["country"] != "All Countries": df_for_brands = df_for_brands[df_for_brands[hierarchy_cols["Country"]] == m_br["country"]]
        m_br["customer"] = c2.selectbox("🏢 Customer", ["All Customers"] + sorted(df_for_brands[hierarchy_cols["Customer"]].replace("", pd.NA).dropna().unique().tolist()), key="br_customer")
        m_br["category"] = c3.selectbox("📦 Category", ["All Categories"] + sorted(df_for_brands[hierarchy_cols["Cat"]].dropna().unique().tolist()), key="br_category")
        m_br["brand"] = c4.selectbox("🏷️ Brand (Licence)", ["All Brands"] + sorted(df_for_brands[hierarchy_cols["Brand"]].dropna().unique().tolist()), key="br_brand_specific")
        m_br["months"] = st.multiselect("📅 Months", MONTHS_ORDER, default=hierarchy_months or MONTHS_ORDER, key="br_months")
        ...
        if m_br["brand"] == "All Brands": st.info("⚠️ Select a specific Brand.")
            ...
            st.divider()
            render_monthly_comparison(v_dfs, m_br, "Net")
            st.divider()
            render_monthly_comparison(v_dfs, m_br, "Qty")
...
with tab_churn:
    ...
    if len([d for d in [df_curr, df_prev, df_old2] if d is not None]) < 2: st.warning("Need 2 years data.")
        ...
        m_ch["country"] = c1.selectbox("🌎 Country", ["All Countries"] + sorted(df_all_ch[hierarchy_cols["Country"]].replace("", pd.NA).dropna().unique().tolist()), key="ch_country")
        m_ch["category"] = c2.selectbox("📦 Category", ["All Categories"] + sorted(df_all_ch[hierarchy_cols["Cat"]].dropna().unique().tolist()), key="ch_category")
        m_ch["brand"] = c3.selectbox("🏷️ Brand", ["All Brands"] + sorted(df_all_ch[hierarchy_cols["Brand"]].dropna().unique().tolist()), key="ch_brand")
        m_ch["months"] = c4.multiselect("📅 Months", MONTHS_ORDER, default=hierarchy_months or MONTHS_ORDER, key="ch_months")
