...
# 🔥 wybór kategorii (z pełnego datasetu)
...
# 🔥 zmiana wartości (KLUCZOWE)
impact["Change Value"] = impact[val26] - impact[val25]

# 🔥 YoY %
impact["YoY %"] = impact.apply(lambda x: yoy_format(calc_yoy(x[val26], x[val25])), axis=1)

# 🔥 usuwamy klientów bez żadnej sprzedaży
impact = impact[(impact[val25] != 0) | (impact[val26] != 0)]

# ================= TOP GROWTH =================
st.write("### 🟢 Top Growth Drivers")

growth = impact[impact["Change Value"] > 0].sort_values("Change Value", ascending=False).head(10)
