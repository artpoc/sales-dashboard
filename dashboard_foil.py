...
def update_cached_file(file_obj, state_key, label):
    if file_obj is None:
        ...
        for key in ["ov_months", "l4l_months", "cr_months", "co_months"]:
...
tab_overview, tab_l4l, tab_full, tab_customer, tab_country = st.tabs(
    [
    ...
    if len(dfs_co) < 1:
        ...
        if not co_valid_dfs_chrono:
            ...
            for d_f, y, c in co_valid_dfs_chrono:
                chart_vals_co.append({"Year": y, "Net": float(sum_decimal(d_f[c["Net"]]))})
            
            if chart_vals_co:
                st.plotly_chart(px.bar(pd.DataFrame(chart_vals_co).sort_values("Year"), x="Year", y="Net", text="Net", color="Year"), use_container_width=True, key="co_net_bar")

            # 2. Country Market Share (Pie Charts)
            st.divider()
            st.markdown("### Country Market Share")
            
            # Grupowanie po krajach na potrzeby Pie i Tabeli
            co_dfs_grouped = []
            for d_f, y, c in co_valid_dfs_chrono:
            ...
            for i, (g, y, _) in enumerate(co_valid_dfs_chrono):
                ...
                with pie_cols_co[i]:
                    st.plotly_chart(px.pie(plot_df, names=c_country, values=f"Net {y}", title=f"Country Share {y}"), use_container_width=True, key=f"co_pie_{y}")


            # 3. Country Performance (L4L)
            st.divider()
            ...
            for y in co_years:
                master_co[f"Net {y}"] = master_co.get(f"Net {y}", pd.Series(dtype=int)).apply(to_display_num)
                master_co[f"Qty {y}"] = master_co.get(f"Qty {y}", pd.Series(dtype=int)).apply(to_display_num)
                
            master_co = master_co.rename(columns={c_country: "Country"})
            display_cols_co[0] = "Country"
            
            st.dataframe(add_index(master_co[display_cols_co]), use_container_width=True)

            # 4. Auto Insights
            st.divider()
            st.markdown("### Auto Insights (Country Focus)")
            
            if len(co_dfs_grouped) >= 2:
                ins_master_co = co_dfs_grouped[0][0]
                for g, y, _ in co_dfs_grouped[1:]:
                    ins_master_co = pd.merge(ins_master_co, g, on=c_country, how="outer")
                ins_master_co = ins_master_co.fillna(Decimal('0'))
                ...
                ins_master_co["YoY"] = ins_master_co.apply(lambda x: yoy_calc(x.get(f"Net {y_newest}", 0), x.get(f"Net {y1}", 0)), axis=1)
                
                ins_master_co[f"Change {y_newest} vs {y1}"] = ins_master_co["Change_Raw"].apply(to_display_num)
                ins_master_co[f"YoY {y_newest} vs {y1} (%)"] = ins_master_co["YoY"].apply(yoy_label)
