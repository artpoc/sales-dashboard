...
if len(all_loaded_dfs_for_colors) > 0:
    ...
    # Stałe kolory dla poszczególnych lat (Niebieski -> Pomarańczowy -> Zielony -> ...)
    year_colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"]
    for i, y in enumerate(all_years):
        GLOBAL_COLOR_MAP[y] = year_colors[i % len(year_colors)]
        
    # Bardzo duża i zróżnicowana paleta barw (74 kolory połączone ze sobą) do unikalnego przypisywania
    palette = px.colors.qualitative.Alphabet + px.colors.qualitative.Light24 + px.colors.qualitative.Dark24
    
    c_idx = 0
    for item in all_categories + all_brands + all_countries:
