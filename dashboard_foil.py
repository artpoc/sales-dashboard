...
with tab_overview:
    ...
    if len(loaded_dfs) < 1:
        ...
        render_three_year_analysis(base_cols["Customer"], "Customer Comparison", dfs_cust, base_years, base_col_maps, "Customer", show_pie=False)
        ...
        if len(ins_dfs_ov) >= 2:
            ...
            if len(ov_years) == 2:
                ...
                display_cols = disp_prefix + [f"Net {ov_years[0]}", f"Net {ov_years[1]}", f"Change {y_newest} vs {y1}", f"YoY {y_newest} vs {y1} (%)"]
            elif len(ov_years) == 3:
                y0 = ov_years[0]
                ...
                display_cols = disp_prefix + [f"Net {y0}", f"Net {y1}", f"Net {y_newest}", f"Change {y_newest} vs {y1}", f"YoY {y_newest} vs {y1} (%)", f"Change {y_newest} vs {y0}", f"YoY {y_newest} vs {y0} (%)"]
        ...
        if meta_cr["customer"] == "All Customers":
            ...
            def render_cr_l4l_table():
                ...
                if l4l_dfs:
                    ...
                    if len(cr_years) == 2:
                        ...
                        display_cols.extend([f"Net {y1}", f"Net {y_newest}", f"Qty {y1}", f"Qty {y_newest}", f"YoY {y_newest} vs {y1} (%)"])
                    elif len(cr_years) == 3:
                        y0, y1 = cr_years[0], cr_years[1]
                        master_l4l[f"YoY {y_newest} vs {y1} (%)"] = master_l4l.apply(lambda x: yoy_label(yoy_calc(x.get(f"Net {y_newest}", 0), x.get(f"Net {y1}", 0))), axis=1) if not master_l4l.empty else []
                        master_l4l[f"YoY {y_newest} vs {y0} (%)"] = master_l4l.apply(lambda x: yoy_label(yoy_calc(x.get(f"Net {y_newest}", 0), x.get(f"Net {y0}", 0))), axis=1) if not master_l4l.empty else []
                        display_cols.extend([f"Net {y0}", f"Net {y1}", f"Net {y_newest}", f"Qty {y0}", f"Qty {y1}", f"Qty {y_newest}", f"YoY {y_newest} vs {y1} (%)", f"YoY {y_newest} vs {y0} (%)"])
            ...
            if len(ins_dfs) >= 2:
                ...
                if len(cr_years) == 2:
                    ...
                    display_cols = disp_prefix + [f"Net {cr_years[0]}", f"Net {cr_years[1]}", f"Change {y_newest} vs {y1}", f"YoY {y_newest} vs {y1} (%)"]
                elif len(cr_years) == 3:
                    y0 = cr_years[0]
